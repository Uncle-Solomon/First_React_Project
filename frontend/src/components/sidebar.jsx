      ...
      <div className="flex flex-col">
        ...
        <div className="flex flex-col gap-5 ">
          ...
          {categories.slice(0, categories.length-1).map((category)=>(
            ...
            onClick={handleCloseSidebar} key={category.name}>{category.name}</NavLink>
          ))}
        </div>
        {user && (
          <Link to={`user-profile/${user._id}`}
          className="flex my-5 mb-3 bottom-0 gap-2 p-2 items-center bg-white rounded-lg shadow-lg mx-3">
